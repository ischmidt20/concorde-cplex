...
/****************************************************************************/
/*                                                                          */
/*              Interface Routines to The CPLEX 6.0-7.0 LP Solvers          */
/*                                                                          */
/*  NOTE: Use this code in place of lp_none.c to access the Cplex 6.0-7.0   */
...
int CClp_status (CClp *lp, int *status)
{
    ...
    if (solmethod == CPX_ALG_PRIMAL || solmethod == CPX_ALG_DUAL) {
        solstat = CPXgetstat (lp->cplex_env, lp->cplex_lp);
        if (solstat == CPX_STAT_OPTIMAL || solstat == CPX_STAT_OPTIMAL_INFEAS) {
            *status = 0;
            return 0;
        } else if (solstat == CPX_STAT_INFEASIBLE && solmethod==CPX_ALG_DUAL) {
            *status = 1;
            return 0;
        } else {
